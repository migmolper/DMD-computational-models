...
def select_free_surface_H(frame, data):
    ...
    data.particles_.create_property("chemical-multp-bcc", dtype=int, data=sel_1 * sel_2)
...
output_file_name = "Mg-hcp-circ-nanowire-R-{}.dump".format(R)

#input_file_name = "../data/MgHx-unitcell-hcp.dump"
#output_file_name = "MgHx-hcp-circ-nanowire-R-{}.dump".format(R)

#input_file_name = "../data/MgHx-unitcell-rutile.dump"
#output_file_name = "MgH2-rutile-circ-nanowire-R-{}.dump".format(R)
...
# Create big block and sculpt it using a cylinder
pipeline.modifiers.append(
    ReplicateModifier(num_x=Num_periodic_x, num_y=Num_periodic_y, num_z=Num_periodic_z)
)

# Sculpt cylinder using cut-planes
def cut_cylinder(frame, data):
    sel = (
        np.sqrt(
...
pipeline.modifiers.append(cut_cylinder)
